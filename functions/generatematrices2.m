...
function [nomefile]=generatematrices2(n,m,Tr,maxt,ac,ad,thor,maxsoc,minsoc)

DataFolder=setDataFolder();
hashcell={n,m,Tr,maxt,ac,ad,thor,maxsoc,minsoc};
nomefile=[DataFolder 'optimatrices/' DataHash(hashcell)];
...
A=sparse([[[sparse(A1);sparse(A2);sparse(A3)] sparse(varno,m*(v>=2))];sparse(A4)]);
...
for j=1:thor        % j = tau
    ...
    AB0=[AB0 sparse(size(B,1),size(B,2)*(thor-j))];
    Ac0=A^j;
    AC0=sparse([[repmat(eye(n^2,'logical'),1,j) sparse(n^2,n^2*(thor-j))];sparse(size(B,1)-dveclength,thor*n^2)]);    % to multiply with C vector size(B,1) x n^2*thor
    ...
    beq=[beq;zeros(size(tmp5,1),size(A,2))];
...
save(nomefile,'A','B','Adis','bdis','bdisc','bdisC','Aeq','beq','beqc','intcon','lb','ub','fu','fx','fq','fqv2g','fsoc','-v7.3');
