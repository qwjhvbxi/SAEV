...
function [X,prices,demand]=RelocationPricing4(m)
...
ub=[repmat(m.v,n,1)*Relocation; pmax;    a];
...
%         H(1:n^2*3+1:end)=0.0001;
    % H=sparse(n^2*3,n^2*3);
...
%     x0=[zeros(1,n^2),(pmin+(pmax-pmin)/2)',zeros(1,n^2)];
%     [X0,fval,Exitflag] = fmincon(myobjfun, x0, A, b, [], [], lb, ub,[],options);%, @myg)

    % optimization
    options=optimoptions('quadprog','display','iter');
    [X0,fval,Exitflag]=quadprog(H,f,A,b,[],[],lb,ub,[],options);
...
%     n=length(m.c);
%     
%     fun=@(x) x*[ c*C ; zeros(n^2,1) ;  c.*C  ]-(c'.*x(n^2+1:n^2*2))*x(n^2*2+1:n^2*3)';
% 
% %     options=optimoptions('ga','display','iter');
% %     [X0,fval,Exitflag]=ga(fun,n^2*3,A,b,[],[],lb,ub,[],options);
% 
% 
%     x0=[zeros(1,n^2),(pmin+(pmax-pmin)/2)',zeros(1,n^2)];
%     options=optimoptions('patternsearch','display','iter');
%     [X0,fval,Exitflag]=patternsearch(fun,x0,A,b,[],[],lb,ub,[],options);
